...
export default function ProductDetailPage() {
  ...
  if (!product) return (
    <div style={{ minHeight: '100vh', paddingTop: 120, textAlign: 'center' }}>
      <p style={{ fontFamily: "'Playfair Display',serif", fontSize: 22, color: '#2B2B2B' }}>Product not found</p>
      <Link href="/products" style={{ color: '#E6C97A', textDecoration: 'none', marginTop: 12, display: 'inline-block' }}>← Back</Link>
    </div>
  );
      ...
      <div className="mx-auto max-w-[1280px] px-4 sm:px-6" style={{ paddingLeft: 'var(--container-px)', paddingRight: 'var(--container-px)' }}>
        ...
        <div className="mb-16 grid grid-cols-1 gap-10 lg:grid-cols-2 lg:gap-16 lg:mb-24">
          ...
          <motion.div initial={{ opacity: 0, x: -32 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.55 }}
            ...
            {product.badge && (
              <span className="badge badge-gold absolute left-4 top-4 sm:left-6 sm:top-6">{product.badge}</span>
            )}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 32 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.55 }} className="flex flex-col justify-center">
            ...
            <h1 className="mb-4 font-serif text-3xl font-bold text-ink sm:text-4xl lg:text-5xl lg:leading-tight">{product.name}</h1>

            <div className="mb-5 flex items-center gap-1">
              {Array.from({ length: 5 }).map((_, i) => <Star key={i} size={14} fill="#E6C97A" color="#E6C97A" />)}
              <span className="ml-2 font-sans text-[13px] text-[#AFAFAF]">(12 reviews)</span>
            </div>

            <div className="divider-gold mb-6" />

            <p className="mb-8 font-sans text-base leading-relaxed text-subtle sm:text-lg">{product.description}</p>

            <p className="mb-8 font-serif text-4xl font-bold text-ink sm:text-5xl">${product.price}</p>

            <div className="mb-8 flex flex-col gap-4 sm:flex-row">
              <button onClick={() => addItem(product)} className="btn-primary flex-1 justify-center py-4">
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-3 sm:gap-3">
              {[[<Shield size={16} />, 'Quality Guaranteed'], [<Truck size={16} />, 'Worldwide Shipping'], [<RefreshCw size={16} />, '14-Day Returns']].map(([ic, lbl], i) => (
        ...
        {related.length > 0 && (
          <div className="border-t border-beige pt-16 sm:pt-24">
            <h2 className="mb-8 font-serif text-2xl font-bold text-ink sm:text-3xl sm:mb-12">You might also love</h2>
