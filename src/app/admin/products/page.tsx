...
export default function AdminProductsPage() {
  const { products, deleteProduct, updateProduct, reorderProducts, categories } = useAdminStore();
  ...
  const canReorder = categoryFilter === 'All' && !searchTerm.trim();
  ...
  const moveProduct = async (id: string, direction: -1 | 1) => {
    ...
    const orderedIds = products.map((item) => item.id);
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Could not reorder products');
    } finally {
    ...
    <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-200">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-[14px] text-[#7A7A7A] mt-1">
            Manage catalog, pricing, and display order on the website.
          </p>
        </div>
        <Link
      ...
      {!canReorder && (
        <p className="text-[12px] text-[#AFAFAF]">
          Clear search and category filters to change product order.
        </p>
      )}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#F8F5F2] border-b border-[#EADFD8]">
                <th className="py-4 px-4 text-[11px] font-bold text-[#AFAFAF] uppercase tracking-wider w-16">
                  Order
                </th>
                <th className="py-4 px-6 text-[11px] font-bold text-[#AFAFAF] uppercase tracking-wider">
                  Product
                </th>
                <th className="py-4 px-6 text-[11px] font-bold text-[#AFAFAF] uppercase tracking-wider">
                  Category
                ...
                <th className="py-4 px-6 text-[11px] font-bold text-[#AFAFAF] uppercase tracking-wider text-center">
                  Status
                </th>
                <th className="py-4 px-6 text-[11px] font-bold text-[#AFAFAF] uppercase tracking-wider text-right">
                  Actions
                </th>
              </tr>
            </thead>
                  ...
                  <tr key={product.id} className="hover:bg-[#F8F5F2]/50 transition-colors group">
                    <td className="py-4 px-4">
                      <div className="flex flex-col items-center gap-0.5">
                        <button
                          type="button"
                          disabled={!canReorder || fullIndex <= 0 || reorderingId === product.id}
                          onClick={() => void moveProduct(product.id, -1)}
                          className="p-1 rounded-md text-[#AFAFAF] hover:text-[#2B2B2B] hover:bg-[#F8F5F2] disabled:opacity-30 disabled:cursor-not-allowed"
                          aria-label="Move up"
                        >
                          <ChevronUp size={16} />
                        </button>
                        <span className="text-[11px] font-medium text-[#AFAFAF] tabular-nums">
                          {canReorder ? fullIndex + 1 : filteredIndex + 1}
                        </span>
                        <button
                          type="button"
                          disabled={
                        ...
                        <div>
                          <p className="text-[14px] font-bold text-[#2B2B2B] flex items-center gap-1.5">
                            ...
                            {product.isFavorite ? (
                              <Heart
                                size={12}
                                className="text-[#E6C97A] fill-[#E6C97A]"
                                aria-label="Favorite"
                              />
                            ) : null}
                          </p>
                          <p className="text-[11px] text-[#AFAFAF]">
                            {pickLocalized(product.name, 'en')}
                          </p>
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex items-center justify-end gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                        <Link
                          ...
                          className="p-2 text-[#AFAFAF] hover:text-[#2B2B2B] hover:bg-[#F8F5F2] rounded-lg transition-colors"
                        >
                          <Edit size={16} />
                        </Link>
                        <button
                          onClick={() =>
                            handleDelete(product.id, pickLocalized(product.name, 'hy'))
                          }
